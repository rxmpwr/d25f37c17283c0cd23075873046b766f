...
class YouTubeCollector:
    ...
    def collect_channel_data(
        ...
        for channel_url in channel_urls:
            ...
            for i, video in enumerate(videos):
                ...
                if video_details:
                    all_data['videos'].append(video_details)
    ...
    def _generate_summary(self, data: Dict) -> Dict:
        ...
        summary = {
            ...
            'total_views': sum(video.get('view_count', 0) for video in data.get('videos', [])),
            'total_likes': sum(video.get('like_count', 0) for video in data.get('videos', [])),
...
class YouTubeDataProcessor:
    ...
    def format_for_display(data: Dict) -> Dict:
        ...
        videos = sorted(data.get('videos', []), key=lambda x: x.get('view_count', 0), reverse=True)
        ...
        # Get recent comments
        comments = sorted(data.get('comments', []), key=lambda x: x.get('like_count', 0), reverse=True)
